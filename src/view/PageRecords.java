...
public class PageRecords {
	...
	public void createButtons(Shell shell, Table table, List<Tournament> tournamentList) {
            ...
            public void widgetSelected(SelectionEvent e) {
            	if (numberOfCurrentPage == 1) {
            		...
            		if (!countPages.getText().isEmpty()) {
            			table.removeAll();
            			count = Integer.parseInt(countPages.getText());
            			setRecordList(tournamentList, table, (numberOfCurrentPage - 2) * count, (numberOfCurrentPage - 1) * count);
            			countRecords.setText("Records on page: " + count);
            			allPages.setText("Pages at all: " + (int)Math.ceil((double)tournamentList.size() / (double)count));
            			numberOfCurrentPage--;
            			allRecords.setText("Records at all: " + tournamentList.size());
            ...
            public void widgetSelected(SelectionEvent e) {
            	if (tournamentList.size() - numberOfCurrentPage * count <= 0) {
            		...
            		if (!countPages.getText().isEmpty()) {
            			table.removeAll();
            			count = Integer.parseInt(countPages.getText());
            			if (count <= tournamentList.size() - numberOfCurrentPage * count) {
            				setRecordList(tournamentList, table, numberOfCurrentPage * count, numberOfCurrentPage * count + count);
            				countRecords.setText("Records on page: " + count);
            				allPages.setText("Pages at all: " + (int)Math.ceil((double)tournamentList.size() / (double)count));
            			} else {
            				setRecordList(tournamentList, table, numberOfCurrentPage * count, tournamentList.size());
            				countRecords.setText("Records on page: " + (tournamentList.size() - numberOfCurrentPage * count));
            				allPages.setText("Pages at all: " + (int)Math.ceil((double)tournamentList.size() / (double)count));
            			}
            			allRecords.setText("Records at all: " + tournamentList.size());
            			numberOfCurrentPage++;
        ...
        lastPage.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
            	if (numberOfCurrentPage == (int)Math.ceil((double)tournamentList.size() / (double)count)) {
            		return;
            	} else {
            		if (!countPages.getText().isEmpty()) {
            			table.removeAll();
            			count = Integer.parseInt(countPages.getText());
            			numberOfCurrentPage = (int)Math.ceil((double)tournamentList.size() / (double)count);
           				setRecordList(tournamentList, table, (numberOfCurrentPage - 1) * count, tournamentList.size());
